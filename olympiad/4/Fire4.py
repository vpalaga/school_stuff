...
class Game:
    ...
    def calc_time(self):
        return find_path( self.fire_tree, -1, self.m)
...
def find_path(data, start, end):
    ...
    while current_node != end:
        ...
        best_node = next(iter(data[current_node]))
        best_w = data[current_node][best_node]

        for node, w in data[current_node].items():
            if w < best_w and node not in path:
                best_w = w
                best_node = node
